...
### 0.5. 네트워크 전제조건 ⚠️ (여기서 반나절 잃었다)

**게이트웨이와 서버 PC 는 같은 유선 세그먼트에 있어야 한다.** 강남 고트의원 6F 실측 결과:
...
즉 **원내 WiFi 로는 게이트웨이를 붙일 수 없고, 서버를 WiFi 에 두면 유선 게이트웨이와도
통신이 안 된다.** 무선 격리는 환자 단말이 원내 기기를 못 건드리게 하는 정상적인 보안 설정이라
그대로 두는 게 맞다 — 우리가 유선으로 가면 된다.

증상이 "장비 불량" 처럼 보이는 게 함정이다. 게이트웨이는 ARP·DHCP 만 응답하고 ping·HTTP·
SSDP 전부 무응답이라 죽은 것처럼 보인다. 진단은 **다른 기기에도 ping 이 안 되는지** 보면 5초다.

WiFi 로 운영하려면 IT 에 요청할 것: 2.4GHz 전용 · WPA2-PSK · **단말 간 통신 허용** ·
업무망과 VLAN 분리한 게이트웨이 전용 SSID. 유선으로 갈 거면 필요 없다.
