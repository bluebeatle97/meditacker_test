...
npm run stop && npm run dev:all                         # 8. 서버 재시작 (필수)
```

⚠️ **8번을 빼먹지 말 것.** 서버는 격자를 시작할 때 한 번 읽어 메모리에 들고 있다.
...
```bash
python tools/build-walkable.py     # "앵커가 막힌 셀에 있는 존" 경고가 뜨는지
npx vitest run                     # 경로·존 테스트
```

- **존 앵커 46개가 전부 통행 가능한가** — 막히면 그 방은 위치 판정이 안 된다
- **존끼리 서로 연결되는가** — 벽이 두꺼워지면 문간이 봉해져 방이 통째로 갇힌다
  (실제로 상담실 4가 갇힌 적이 있다. 원인은 벽 **그림자**까지 벽으로 센 것)
- **실좌표가 격자 안인가** — 서버 재시작 후 `pos:update` 를 받아 `walkable` 과 대조
