...
| 주소 | 기대 |
|---|---|
| `/` | 핀 상자가 뜬다 → 핀 넣으면 도면. **기본은 실장비만이라 사람이 없다** — 우상단 **🧪 테스트 장비 켜기** 를 누르면 가상 인원이 걸어다닌다. **'시연용 가상 데이터' 배지가 없어야** 진짜 서버에 붙은 것 |
...
> 다시 수렴하고, 테스트 장비 토글이 기본값(실장비)으로 돌아간다. 급하면 커밋 **제목 줄**에
> `[no deploy]` 를 넣는다 — 이미지는 만들고 배포만 건너뛴다. (본문이 아니라 제목이다.)
